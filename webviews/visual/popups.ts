...
export function showPopup(x: number, y: number): HTMLElement {
  ...
  requestAnimationFrame(() => {
    ...
    // Vertically the popup takes what its content needs; the window edge is the
    // only cap. The clamp lands on the inner scroller rather than the popup, so
    // the border and padding stay put and the scrollbar belongs to the list.
    // The floor keeps a menu opened near the bottom edge usable — the page
    // scrolls to the rest of it, as it always did.
    const rect = pop.getBoundingClientRect();
    const room = document.documentElement.clientHeight - rect.top - 8;
    if (rect.height > room) {
      const scroller = pop.querySelector<HTMLElement>(".vmenu") ?? pop;
      const chrome = rect.height - scroller.offsetHeight;
      scroller.style.maxHeight = `${Math.max(200, room - chrome)}px`;
