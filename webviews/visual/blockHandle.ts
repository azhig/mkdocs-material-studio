...
export function initBlockHandle(next: BlockHandleHost): void {
  ...
  host.docEl.addEventListener("mousedown", (e) => {
    ...
    activateBlock(handleBlockOf(e.target as Node));
  });
...
export function handleBlockOf(node: Node | null): HTMLElement | null {
  ...
  while (cur && cur !== host.docEl) {
    ...
    if (cur.hasAttribute("data-src-line") && isHandleContainer(parent)) {
      return cur as HTMLElement;
    }
    cur = parent;
  }
...
/** Makes a block active (the handle plus the highlighting). */
...
function blockManageItems(block: HTMLElement): QuickItem[] {
  ...
  items.push({ label: `⧉ ${t("Copy")}`, run: () => void copyBlock(block) });
  items.push({ label: `✂ ${t("Cut")}`, run: () => void cutBlock(block) });
...
// Our own block clipboard. The system one is filled too (it can be pasted into any editor),
// but a webview is not always allowed to read it — on paste it is only the first source.
let blockClipboard = "";
...
async function copyBlock(block: HTMLElement): Promise<void> {
  ...
  try {
    ...
    /* the system clipboard is unavailable — our own one is enough */
  }
  st.set(t("Block copied"));
}

async function cutBlock(block: HTMLElement): Promise<void> {
  const text = blockSourceText(block);
  if (!text) {
    return;
  }
