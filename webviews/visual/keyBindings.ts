...
/** What claiming a keystroke needs of an event — spelled out so the rule can be tested without a DOM. */
export interface ClaimableEvent extends HotKeyEvent {
  preventDefault(): void;
  stopPropagation(): void;
}

/**
 * Keeps a keystroke inside the webview.
 *
 * `stopPropagation` is the load-bearing half. VS Code's webview preload
 * forwards every keydown to the extension host from `window`, on the bubble
 * phase, and never looks at `defaultPrevented` — so a shortcut we handled would
 * ALSO run VS Code's own command for it: Ctrl/Cmd+B made text bold *and*
 * collapsed the side bar. Our listeners sit below `window`, so refusing to
 * bubble is the only thing that takes the key for good. It is deliberately
 * called from the branches that actually ran something: a key we do not own
 * must still reach VS Code.
 */
export function claimKey(e: Pick<ClaimableEvent, "preventDefault" | "stopPropagation">): void {
  e.preventDefault();
  e.stopPropagation();
}

/**
 * Runs the command bound to this keystroke and says whether there was one. The
 * lookup goes through the registry, so it follows the user's reassignments —
 * and so does the shielding above, which is why no list of VS Code's own
 * shortcuts has to be maintained anywhere.
 */
export function consumeHotKey(e: ClaimableEvent): boolean {
  const hk = eventHotKey(e);
  const command = hk ? keyIndexes().byHotKey.get(hotKeyToString(hk)) : undefined;
  if (!command) {
    return false;
  }
  claimKey(e);
