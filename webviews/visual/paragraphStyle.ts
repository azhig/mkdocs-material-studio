...
/**
 * The card of a Material grid, when the item is one. In markup a card grid is a
 * plain list inside `div.grid.cards`, but to the author a card is a card, not a
 * bullet: the toolbar must not call it a list, and the list operations must not
 * touch the cards themselves — un-listing them dissolves the grid into
 * paragraphs.
 */
export function gridCardOf(li: HTMLElement): HTMLElement | null {
  const list = li.parentElement;
  const cards = (el: Element | null): boolean =>
    el !== null && el.classList.contains("grid") && el.classList.contains("cards");
  return list && (cards(list) || cards(list.parentElement)) ? li : null;
}
...
function currentListKind(): ListKind | null {
  ...
  return li && !gridCardOf(li) ? listKindOf(li) : null;
}
/**
...
export function applyList(kind: ListKind): void {
  const li = caretListItem();
  const card = li ? gridCardOf(li) : null;
  const target = li && !card ? host.rangedAncestor(li.parentElement) : null;
  if (li && target) {
    ...
    let { start, end } = range;
    if (card) {
      // Inside a card the selection reaches the card's own marker line — the
      // title lives on it. That line is the grid's, not the text's: the new
      // list starts below it and never crosses into the next card.
      const own = rangeOf(card);
      if (!Number.isFinite(own.start)) {
        return;
      }
      start = Math.max(start, own.start + 1);
      end = Math.min(end, own.end);
      while (start < end && (docLines()[start] ?? "").trim() === "") {
        start++;
      }
      while (end > start && (docLines()[end - 1] ?? "").trim() === "") {
        end--;
      }
      if (start >= end) {
        return;
    ...
    const src = docLines().slice(start, end);
    host.replaceLines(start, end, toList(src, kind, host.indentOfLine(start)));
  }
