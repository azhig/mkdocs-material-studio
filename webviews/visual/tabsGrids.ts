...
/**
 * Which tab is open in each set, by the set's position in the document.
 *
 * Markdown says nothing about the open tab, so every render shows the first
 * one. Without carrying the choice across, any structural edit anywhere on the
 * page — moving a block, inserting a component, changing a list — throws the
 * reader back to tab 1 of every set they had opened.
 */
export function openTabs(root: ParentNode): number[] {
  return Array.from(root.querySelectorAll<HTMLElement>(".tabbed-set"), (set) => {
    const found = tabSetParts(set).inputs.findIndex((input) => input.checked);
    return found < 0 ? 0 : found;
  });
}

/** Puts those tabs back after the DOM has been rebuilt. */
export function restoreOpenTabs(root: ParentNode, open: number[]): void {
  Array.from(root.querySelectorAll<HTMLElement>(".tabbed-set")).forEach((set, i) => {
    const input = tabSetParts(set).inputs[open[i] ?? 0];
    if (input) {
      input.checked = true;
    }
  });
...
export function wireTabControls(set: HTMLElement): void {
  ...
  for (const label of Array.from(labelsBox.querySelectorAll<HTMLElement>(":scope > label"))) {
    ...
    // Switching tabs has to take the caret with it. The label is tied to a radio
    // button, so a click changes what is on screen and nothing else: the caret
    // stays in the tab you came from, and the next insert — a button, a table —
    // lands there, out of sight, with nothing to explain why.
    label.addEventListener("click", () => {
      if (label.getAttribute("contenteditable") === "true") {
        return; // the label is being renamed, not switched to
      }
      const parts = tabSetParts(set);
      const block = parts.blocks[parts.labels.indexOf(label)];
      if (block) {
        host.caretIntoBlock(block);
      }
    });
...
/**
 * Enter inside a card: a new paragraph INSIDE it, never a new card. Left to the
 * browser, Enter in a list item splits the item — for a grid that quietly mints
 * a card, and a freshly added card (a bare title) could never be given a body.
 * The split is done in the DOM, like the native one, and the serializer writes
 * the item back as a loose card; a caret mid-text carries the tail with it.
 */
export function splitCardParagraph(card: HTMLElement): HTMLElement | null {
  ...
  const par = at?.closest("p");
  const next = document.createElement("p");
  if (par && card.contains(par) && par.contains(range.startContainer)) {
    // Both ends inside the paragraph, so extracting never crosses its boundary
    // (a range past the element would split the <p> into a duplicate).
    const tail = document.createRange();
    tail.selectNodeContents(par);
    ...
    // A tight card keeps its text right in the <li> — the new paragraph goes last.
    card.appendChild(next);
  }
  if ((next.textContent ?? "") === "" && !next.querySelector("br")) {
