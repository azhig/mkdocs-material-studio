...
export interface InsertsHost {
  ...
  pickFile(kind: "image" | "any"): Promise<string>;
...
export function openAdmonitionInsert(): void {
  ...
  ok.addEventListener("click", () => {
    ...
    const title = ADMONITION_LABELS[selType] ?? selType;
    closePopup();
...
export function openSnippetInsert(): void {
  ...
  browse.addEventListener("click", () => {
    void host.pickFile("any").then((rel) => {
      if (rel) {
        input.value = rel;
      }
    });
  ...
  help.textContent = t("The file's content is inserted by MkDocs when the site is built.");
