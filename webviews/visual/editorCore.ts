...
/**
 * A block the engine drew by itself: the footnote tail, a `<details>` written
 * as raw HTML, whatever a project's extensions add. No line of the file
 * produced it, so it has no source line — and that is exactly what a paragraph
 * the author has just typed looks like. Telling the two apart matters here more
 * than anywhere: the plan below writes new blocks into the file, and one of
 * these would be written in as if the author had typed it.
 *
 * The mark is put on at render time (markServiceBlocks); the footnote classes
 * are still recognised on their own, for a page rendered before the mark
 * existed and for the annotation editor's own document.
 */
export function isServiceBlock(el: Element): boolean {
  return (
    el.classList.contains("vservice") ||
    el.classList.contains("footnotes-sep") ||
