...
function admonitionLines(el: Element): string[] {
  ...
  if (!titleEl) {
    ...
    // A title equal to the type is what Material shows anyway, so it is left out
    // — unless the author had written it out, which the render records.
    if (title !== defaultTitle(classes[0]) || el.hasAttribute("data-title-given")) {
      head += ` "${title}"`;
    }
  }
...
/** The title Material shows when the block does not give one. */
...
function listLines(list: Element): string[] {
  ...
  items.forEach((li, index) => {
    if (loose && index > 0 && separatedInSource(items[index - 1], li)) {
      out.push("");
    }
...
/**
 * Were these two items written with a blank line between them? A list turns
 * “loose” as soon as ONE of its items grows a nested block, and the whole list
 * is then written back with the items spread apart — a diff across items the
 * author never touched. Their source ranges say what was actually there; only
 * when the ranges are missing (a freshly typed item) does the shape of the
 * list decide.
 */
function separatedInSource(prev: Element, next: Element): boolean {
  if (!prev.hasAttribute("data-src-end") || !next.hasAttribute("data-src-line")) {
    return true;
  }
  const end = Number(prev.getAttribute("data-src-end"));
  // Anything other than “the next item starts where this one ended” — a gap the
  // author left, or an item dragged here from elsewhere in the file, whose
  // range no longer describes where it is — leaves the items apart.
  if (Number(next.getAttribute("data-src-line")) !== end) {
    return true;
  }
  // Inside a blockquote the separator counts as part of the item it follows, so
  // the items are adjacent and the blank line shows up as the item reaching
  // past the last thing in it.
  const kids = Array.from(prev.children).filter((c) => c.hasAttribute("data-src-end"));
  const last = kids.at(-1);
  return last !== undefined && Number(last.getAttribute("data-src-end")) < end;
}
...
/** What an empty cell is written as, wide enough to be clickable in the source. */
...
function inlineChildren(el: Element, ctx: InlineCtx): string {
  const pieces = Array.from(el.childNodes).map((node) => {
    if (node.nodeType === TEXT) {
      return escapeText(node.nodeValue ?? "", ctx);
    }
    return node.nodeType === ELEMENT ? serializeInline(node as Element, ctx) : "";
  });
  // The emphasis repair belongs here rather than in wrapNonEmpty: whether a
  // “**” can open depends on the character in front of it, and only the parent
  // sees both sides of a child.
  return pieces
    .map((piece, i) => keepEmphasisParseable(piece, endsBefore(pieces, i), startsAfter(pieces, i)))
    .join("");
}

/** The character an inline lands next to — "" at the edge of the line. */
function endsBefore(pieces: string[], i: number): string {
  for (let k = i - 1; k >= 0; k--) {
    if (pieces[k] !== "") {
      return pieces[k].slice(-1);
    }
  }
  return "";
}

function startsAfter(pieces: string[], i: number): string {
  for (let k = i + 1; k < pieces.length; k++) {
    if (pieces[k] !== "") {
      return pieces[k].slice(0, 1);
    }
  }
  return "";
}
...
/**
 * Emphasis whose markers CommonMark reads by the characters around them. The
 * pymdownx ones — `^^`, `~` and `^` — have rules of their own and parse in the
 * places these do not, so they are left alone.
 */
const FLANKING = /^(\s*)(\*\*|==|~~|\*)([\s\S]+?)\2(\{[^}]*\})?(\s*)$/;

const WORD_CHAR = /[\p{L}\p{N}]/u;

/**
 * Moves one character out of an emphasis run when CommonMark would refuse to
 * open or close it there.
 *
 * Selecting `.text` and pressing bold after a word serializes to
 * `Word**.text**`, and a `**` between a letter and a punctuation mark opens
 * nothing — the reader gets the asterisks themselves, which is what this was
 * written for. `Word.**text**` says the same thing and parses, because the
 * marker now follows punctuation. A period that is no longer bold is a
 * difference nobody can see; asterisks in the middle of a sentence are not.
 */
function keepEmphasisParseable(piece: string, prev: string, next: string): string {
  const m = FLANKING.exec(piece);
  if (!m) {
    return piece;
  }
  const [, lead, marker, , attrs = "", tail] = m;
  let core = m[3];
  let before = "";
  let after = "";

  // Whitespace outside the markers already makes them flanking — wrapNonEmpty
  // put it there. Only a marker pressed against a word needs help.
  if (lead === "" && WORD_CHAR.test(prev) && !WORD_CHAR.test(core[0])) {
    // An escape is one character to the reader: “\*” moves as a pair.
    const take = core.startsWith("\\") && core.length > 1 ? 2 : 1;
    before = core.slice(0, take);
    core = core.slice(take);
  }
  if (
    tail === "" &&
    attrs === "" &&
    core !== "" &&
    WORD_CHAR.test(next) &&
    !WORD_CHAR.test(core.slice(-1))
  ) {
    const take = core.length > 1 && core.slice(-2, -1) === "\\" ? 2 : 1;
    after = core.slice(core.length - take);
    core = core.slice(0, core.length - take);
  }
  if (before === "" && after === "") {
    return piece;
  }
  // Everything that was emphasised had to move: an empty pair of markers is not
  // emphasis, it is two asterisks.
  if (core === "") {
    return `${lead}${before}${after}${tail}`;
  }
  return `${lead}${before}${marker}${core}${marker}${attrs}${after}${tail}`;
}
...
function wrapNonEmpty(el: Element, ctx: InlineCtx, marker: string): string {
  ...
  // A line break is not something to emphasise. Switching bold on in an empty
  // paragraph leaves “<b><br></b>” behind, and wrapping that writes two lines of
  // “**” into the file. They come back as literal asterisks, get escaped on the
  // next edit, and the rubbish breeds — see the “\\*\\*” a user found.
  if (inner.replace(/\x00b|<br>/g, "").trim() === "") {
    return inner;
  }
