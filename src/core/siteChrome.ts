...
export async function buildSiteChrome(
  ...
  return {
    ...
    logoUri: config.theme.logo
      ...
      : undefined,
    logoSvg: readIconSvg(extensionUri, config.theme.icon?.logo),
...
function readIconSvg(extensionUri: vscode.Uri, shortcode: string | undefined): string | undefined {
  if (!shortcode) {
    return undefined;
  }
  ...
  // In mkdocs.yml the set is separated by a slash (`material/library`); the pack
  // keys icons the way a shortcode does, with a dash.
  const svg = iconPackFor(extensionUri).get(`${parts[0]}-${parts[1]}`);
  if (svg === undefined) {
