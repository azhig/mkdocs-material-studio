...
function admonitionRule(
  ...
  const lastContent = findIndentedContentEnd(state, startLine + 1, contentIndent, endLine);
  ...
  if (hasQuotes && !suppressed) {
    // `!!! note` and `!!! note "Note"` draw the same block, so the rendered
    // title cannot say which one the author wrote — and the editor rewrites a
    // block from what it drew. Without this mark the quoted form would quietly
    // disappear from the file the first time anybody edited that admonition.
    openToken.attrSet("data-title-given", "");
  }
