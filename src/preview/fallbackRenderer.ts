...
export function createFallbackRenderer(context: vscode.ExtensionContext): FallbackRenderFn {
  const iconsRoot = vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "svg").fsPath;
  const iconCache = new Map<string, string | undefined>();

  const resolveIcon = (shortcode: string): string | undefined => {
    if (iconCache.has(shortcode)) {
      return iconCache.get(shortcode);
    }
    const dash = shortcode.indexOf("-");
    if (dash < 0) {
      iconCache.set(shortcode, undefined);
      return undefined;
    }
    const set = shortcode.slice(0, dash);
    const rest = shortcode.slice(dash + 1);
    const file = path.join(iconsRoot, set, `${rest}.svg`);
    ...
    // Cap the cache so that it does not grow without bound.
  ...
  return async (doc, project, textOverride, toWebviewUri) => {
    // Refresh the base directories for snippets.
    ...
    if (project) {
      try {
        ...
        snippetBases.push(docsRoot);
        snippetBases.push(project.root.fsPath);
        siteRoot = docsRoot;
