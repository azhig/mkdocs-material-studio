...
export class VisualEditorProvider implements vscode.CustomTextEditorProvider {
  ...
  private async onMessage(
    ...
    switch (m.type) {
      ...
      case "openLink":
        ...
        break;
      case "iconNames":
        // The picker lists and searches names; the 14 342 SVGs stay in the pack.
        void panel.webview.postMessage({
          type: "iconNames",
          sets: extensionIconPack(this.context).names(),
        });
        break;
      case "iconSvgs":
        // One message per screenful of the grid, not one request per icon.
        void panel.webview.postMessage({
          type: "iconSvgs",
          // The id comes back with the answer: batches overlap, and the webview
          // matches them by it rather than by arrival order.
          id: Number(m.id),
          svgs: extensionIconPack(this.context).getMany(
            Array.isArray(m.codes) ? m.codes.map((c) => String(c)) : [],
          ),
        });
        break;
    }
  ...
  private async pickFile(
    ...
    const file = picked?.[0];
    let rel = "";
    if (file && kind === "snippet") {
      // A snippet path resolves against the project root (pymdownx's default
      // base), not against the page — a page-relative `../x.md` renders as
      // “Snippet not found”. The base list mirrors the renderer's.
      const project = await this.projects.findProjectFor(document.uri);
      const bases = project ? [project.root.fsPath] : [docDir];
      const snip = snippetInsertPath(bases, file.fsPath);
      if (snip === undefined) {
        void vscode.window.showWarningMessage(
          t("The file lies outside the project, so a snippet cannot include it."),
        );
      }
      rel = snip ?? "";
    } else if (file) {
...
<div id="vt">
  ...
  <button id="tbComponent" title="${t("Insert a Material component (block or inline)")}"><span class="codicon codicon-add"></span></button>
  <div class="sep"></div>
