...
// A caret needs a text line inside every cell. The engine renders `|   |` as a
// bare <td></td>, and the caret in such a cell has nowhere to stand — Chrome
// parks it visually on the cell's edge, seemingly between cells. This is what
// a user hit right after inserting a table.
describe("empty rendered cells", () => {
  it("get a <br> to stand the caret in, and only the empty ones", async () => {
    const { padEmptyCells } = await import("../../webviews/visual/tables");
    const { buildMarkdownEngine } = await import("../../src/preview/markdownEngine");
    const md = buildMarkdownEngine({ resolveIcon: () => undefined, readSnippet: () => undefined });
    const host = document.createElement("div");
    host.innerHTML = md.render("| Alpha |  |\n| --- | --- |\n|  | beta |\n");
    padEmptyCells(host);
    const cells = Array.from(host.querySelectorAll("th, td"));
    expect(cells.map((c) => c.innerHTML)).toEqual(["Alpha", "<br>", "<br>", "beta"]);
  });

  it("changes nothing in the file: the serializer strips the trailing <br>s", async () => {
    const { padEmptyCells } = await import("../../webviews/visual/tables");
    const { serializeTopBlock } = await import("../../webviews/visual/htmlToMd");
    const { buildMarkdownEngine } = await import("../../src/preview/markdownEngine");
    const md = buildMarkdownEngine({ resolveIcon: () => undefined, readSnippet: () => undefined });
    const src = "| Alpha |  |\n| --- | --- |\n|  | beta |\n";
    const host = document.createElement("div");
    ...
    const before = serializeTopBlock(table);
    padEmptyCells(host);
    expect(serializeTopBlock(table)).toBe(before);
  });
});
