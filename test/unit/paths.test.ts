...
import { readSnippetFrom, snippetInsertPath } from "../../src/preview/snippetSource";
...
describe("snippetInsertPath", () => {
  const root = path.resolve("/project");

  it("writes the path from the project root, POSIX slashes", () => {
    expect(snippetInsertPath([root], path.join(root, "CLAUDE.md"))).toBe("CLAUDE.md");
    expect(snippetInsertPath([root], path.join(root, "docs", "parts", "note.md"))).toBe(
      "docs/parts/note.md",
    );
  });

  it("what it writes, readSnippetFrom will find", () => {
    const file = path.join(root, "includes", "abbr.md");
    const rel = snippetInsertPath([root], file)!;
    expect(readSnippetFrom([root], rel, (f) => (f === file ? "text" : undefined))).toBe("text");
  });

  it("refuses a file outside every base — such an include can never be read", () => {
    expect(snippetInsertPath([root], path.resolve("/elsewhere/readme.md"))).toBeUndefined();
  });

  it("the first base containing the file wins", () => {
    const docs = path.join(root, "docs");
    expect(snippetInsertPath([docs, root], path.join(docs, "note.md"))).toBe("note.md");
    expect(snippetInsertPath([docs, root], path.join(root, "mkdocs.yml"))).toBe("mkdocs.yml");
  });
});
