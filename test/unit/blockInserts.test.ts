...
describe("admonitions", () => {
  ...
  it("inserts a note with its title and a body line by default", () => {
    h.inserts.openAdmonitionInsert();
    press("Insert");
    expect(h.inserted).toEqual(['!!! note "Note"\n    Text.']);
  });

  it("the chosen type becomes both the keyword and the title", () => {
    h.inserts.openAdmonitionInsert();
    popup().querySelector<HTMLElement>(".adm-type.adm-warning")!.click();
    press("Insert");
    expect(h.inserted).toEqual(['!!! warning "Warning"\n    Text.']);
  });
  ...
  it("collapsible states use the markers Material expects", () => {
    for (const [button, marker] of [
      ...
      press(button);
      press("Insert");
      expect(h.inserted[0].startsWith(`${marker} note `)).toBe(true);
    }
  });
