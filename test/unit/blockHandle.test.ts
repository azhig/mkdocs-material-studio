...
describe("which block a node belongs to", () => {
  ...
  it("is the call-out itself when the caret sits on its title", () => {
    // The title paragraph carries the range of the `!!!` marker line. As a
    // block of its own, “Delete block” on it cuts that line and beheads the
    // call-out — the body is left behind as an indented code block. This is
    // what a user hit inside a tab.
    load('!!! note "Title"\n\n    Body of the note.\n');
    const title = pick(".admonition-title");
    expect(handleBlockOf(title.firstChild)).toBe(pick(".admonition"));
  });

  it("is the call-out itself on the title of one nested in a tab", () => {
    load('=== "Tab"\n\n    !!! note "Title"\n\n        Body of the note.\n');
    const title = pick(".admonition-title");
    expect(handleBlockOf(title.firstChild)).toBe(pick(".admonition"));
  });

  it("is the collapsible call-out itself on its summary", () => {
    load('??? note "Title"\n\n    Body of the note.\n');
    const summary = pick("summary");
    expect(handleBlockOf(summary.firstChild)).toBe(pick(".admonition"));
  });
